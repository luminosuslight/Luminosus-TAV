...
Although IntelliTest generated 6 different tests, which all met our specification, the testing framework reported some issues about floating point numbers. This also seems to be the reason for the low variety of the \texttt{s} and \texttt{v} parameters. Additionally, IntelliTests reported, that the stack observation, used for test case generation, is limited in our case. This might also lead to less test cases being generated overall.
...
Because manual theorem proving can be very hard and time consuming, it is worth to have a look at tools that claim to prove the absence of certain failures without optimistic inaccuracy. In the following section we want to analyze how well this can be done for our code and what additional benefits automatic symbolic execution can have by using the only tool we found for this for modern C++ code, Polyspace.
...
Furthermore Polyspace can show the possible value range of each variable at every given time as a result of the automatic symbolic execution. These information can be very useful for debugging, too, an example can be seen in \vref{fig:polyspace_symbolic_execution}.
